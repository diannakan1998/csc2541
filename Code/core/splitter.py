    ...
    def get_split_data(self): 
        ...
        for fold in range(self.nfolds):
            ...
            for tvt in ['train','valid','test']:
                ...
                get_noise = np.copy(x_new)
                x_new[final_dataset[fold][tvt]['m']==0] = np.nan
                print(get_noise.shape)
                get_noise = np.random.normal(0, 1, get_noise.shape[0])
                get_noise[final_dataset[fold][tvt]['m']!=0] = 0
                x_new_filled  = []
                for k in range(x_new.shape[-1]):
                    x_new_filled.append(pd.DataFrame(x_new[...,k]).fillna(method='ffill', axis=1).values[...,None] + get_noise[...,k].reshape((get_noise.shape[0],get_noise.shape[1],1)))
